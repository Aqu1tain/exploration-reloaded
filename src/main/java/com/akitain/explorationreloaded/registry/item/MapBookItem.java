...
public class MapBookItem extends Item {
    ...
    public Text getName(ItemStack stack) {
        if (stack != null && !this.hasMapBookId(stack)) {
            if (stack.contains(ExplorationComponents.MAP_BOOK_ADDITIONS)) {
                return Text.translatable("item.exploration-reloaded.map_book_new");
            } else {
                return Text.translatable("item.exploration-reloaded.map_book_empty");
            }
        } else {
    ...
    public void appendTooltip(ItemStack stack, Item.TooltipContext context, TooltipDisplayComponent displayComponent, Consumer<Text> textConsumer, TooltipType type) {
        ...
        if (id != -1) {
            ...
            textConsumer.accept(Text.translatable("item.exploration-reloaded.map_book_id", (id + 1)).formatted(Formatting.GRAY));
        }

        if (mapsCount > 0) {
            textConsumer.accept(Text.translatable("item.exploration-reloaded.map_book_maps", mapsCount).formatted(Formatting.GRAY));
        }
    }
