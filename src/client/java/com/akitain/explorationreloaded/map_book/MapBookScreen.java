...
public class MapBookScreen extends Screen {
    ...
    @Override public boolean mouseClicked(MouseButtonEvent click, boolean doubleClick) {
        ...
        if (click.button() == 0 && minecraft.hasShiftDown()) {
            ...
            if (id != -1) {
                ...
                String dim = minecraft.level.dimensionTypeRegistration().getRegisteredName();
    ...
    public static int getColor(MapBookPlayer player, Minecraft client) {
        assert client.player != null;
        int color = ARGB.setBrightness(ARGB.color(255, player.name.hashCode()), 0.9F);
        for (PlayerInfo playerListEntry : client.player.connection.getOnlinePlayers()) {
    ...
    private void renderIcons(GuiGraphicsExtractor context) {
        ...
        for (MapStateData mapStateData : getMapStates(item, minecraft.level)) {
            float render = 0.0f;
            if (minecraft.level.dimensionTypeRegistration().getRegisteredName().contains(mapStateData.mapState.dimension.identifier().toString()))
                render = 1.0f;
            if (minecraft.level.dimensionTypeRegistration().getRegisteredName().contains("the_nether") && mapStateData.mapState.dimension.identifier().toString().contains("overworld"))
                render = 1/8.0f;
            if (render>0) {
